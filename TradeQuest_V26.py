...
def load_universe(selected):
    ...
    if "Quality Swing Universe" in selected:
        symbols |= set(quality_swing_universe())
...
def scan_symbol(symbol, market_regime):
    """V26 swing scanner: exact V19/V20 six-condition strategy."""
...
# CAPITAL
# ----------------------------
st.sidebar.header("⚙️ TradeQuest V26")
...
if scanner_mode == "📈 Swing":
    selected_universe = st.sidebar.selectbox(
        "📊 Swing Universe",
        ["NIFTY 200", "NIFTY 500", "F&O Stocks", "Quality Swing Universe"],
        index=1,
    )
    max_stocks = st.sidebar.slider("Stocks to scan", 50, 2500, 2500, 50)
...
# ============================================================
# ACTIVE TRADE TRACKING — USER SELECTED ONLY
# ============================================================

ACTIVE_TRADES_FILE = Path("tradequest_v26_active_trades.json")
...
def load_active_trades():
    if not ACTIVE_TRADES_FILE.exists():
        return []
    try:
        with ACTIVE_TRADES_FILE.open("r", encoding="utf-8") as f:
            data = json.load(f)
        return data if isinstance(data, list) else []
    except Exception:
        return []
...
def save_active_trades(rows):
    try:
        with ACTIVE_TRADES_FILE.open("w", encoding="utf-8") as f:
            json.dump(rows, f, indent=2, default=str)
    except Exception:
        pass


def trading_days_held(start_date):
    try:
...
def current_price_for_active(symbol, current_results):
    if current_results is not None and not current_results.empty and "Symbol" in current_results.columns:
        m = current_results[current_results["Symbol"].astype(str).str.upper() == clean_symbol(symbol)]
        if not m.empty and "Current" in m.columns:
            p = safe_float(m.iloc[0].get("Current"), np.nan)
            if np.isfinite(p):
                return p
    try:
        d = download_history(symbol, "1mo")
        if not d.empty:
            return safe_float(d.iloc[-1]["Close"], np.nan)
    except Exception:
        pass
    return np.nan


def update_active_trades(current_results):
    rows = load_active_trades()
    active_out = []
    changed = False
    for rec in rows:
        ...
        buy_price = safe_float(rec.get("buy_price"), np.nan)
        qty = int(max(1, safe_float(rec.get("qty"), 1)))
        sl = safe_float(rec.get("sl"), np.nan)
        target = safe_float(rec.get("target"), np.nan)
        current = current_price_for_active(symbol, current_results)
        days = trading_days_held(rec.get("buy_date", datetime.now().strftime("%Y-%m-%d")))

        exit_reason = None
        if np.isfinite(current) and np.isfinite(sl) and current <= sl:
            exit_reason = "🛑 STOP LOSS HIT"
        elif np.isfinite(current) and np.isfinite(target) and current >= target:
            exit_reason = "🎯 TARGET HIT"
        elif days >= MAX_HOLDING_DAYS:
            exit_reason = "⏳ 20 TRADING DAYS COMPLETE"

        if exit_reason:
            rec["status"] = "CLOSED"
            rec["exit_reason"] = exit_reason
            rec["exit_price"] = float(current) if np.isfinite(current) else None
            rec["closed_at"] = datetime.now().isoformat(timespec="seconds")
            changed = True
            continue

        pnl = (current - buy_price) * qty if np.isfinite(current) and np.isfinite(buy_price) else np.nan
        pnl_pct = ((current - buy_price) / buy_price * 100) if np.isfinite(current) and buy_price > 0 else np.nan
        active_out.append({
            "Symbol": symbol,
            "Buy Date": rec.get("buy_date", ""),
            "Buy Price": round(buy_price, 2),
            "Current": round(current, 2) if np.isfinite(current) else np.nan,
            "Qty": qty,
            "P/L ₹": round(pnl, 2) if np.isfinite(pnl) else np.nan,
            "P/L %": round(pnl_pct, 2) if np.isfinite(pnl_pct) else np.nan,
            "SL": round(sl, 2),
            "Target": round(target, 2),
            "Days Held": days,
    ...
    # Keep closed records in the JSON file for audit/history, but do not show them in Active Trades.
    if changed or len(active_out) != sum(1 for x in rows if x.get("status") == "ACTIVE"):
        save_active_trades(rows)
    return active_out


# ============================================================
# 5-YEAR BACKTEST — EXACT SIX CONDITIONS
# ============================================================

@st.cache_data(ttl=3600, show_spinner=False)
def backtest_symbol(symbol):
    df = download_history(symbol, "5y")
    if df.empty or len(df) < 120:
        return []
    d = add_indicators(df).dropna(subset=["MA20", "MA50", "RSI", "MACD", "MACD_SIGNAL", "VOL_RATIO", "HIGH50"])
    if len(d) < 60:
        return []
    ...
    while i < len(d) - 1:
        ...
        volx = safe_float(r["VOL_RATIO"])
        high50 = safe_float(r["HIGH50"])
        if not all(np.isfinite(x) for x in [close,ma20,ma50,rsi,macd,signal,volx,high50]):
            i += 1
            continue
        ready = (
            close > ma20 and ma20 > ma50 and 50 <= rsi <= 70 and
            macd > signal and volx >= VOLUME_MULTIPLIER and close > high50
        )
        if not ready:
            i += 1
            continue
        ...
        entry_i = i + 1
        entry = safe_float(d.iloc[entry_i]["Open"])
        if not np.isfinite(entry) or entry <= 0:
            i += 1
            continue
        sl = entry * (1 - STOP_PERCENT / 100)
        target = entry * (1 + TARGET_PERCENT / 100)
        exit_i = min(entry_i + MAX_HOLDING_DAYS - 1, len(d) - 1)
        exit_price = safe_float(d.iloc[exit_i]["Close"])
        reason = "MAX HOLD"
        for j in range(entry_i, exit_i + 1):
            hi = safe_float(d.iloc[j]["High"])
            lo = safe_float(d.iloc[j]["Low"])
            if np.isfinite(lo) and lo <= sl:
                exit_i = j; exit_price = sl; reason = "STOP LOSS"; break
        ...
        trades.append({
            "Symbol": clean_symbol(symbol),
            "Entry Date": str(d.index[entry_i].date()),
            "Exit Date": str(d.index[exit_i].date()),
            "Entry": round(entry,2),
...
def run_five_year_backtest(symbols, capital, max_positions):
    all_trades = []
    progress = st.progress(0)
    total = max(len(symbols), 1)
    for n, symbol in enumerate(symbols):
        try:
            all_trades.extend(backtest_symbol(symbol))
        except Exception:
            pass
    ...
    if trade_df.empty:
        return {"data_years": "No sufficient history", "trades": 0, "win_rate": 0.0, "pnl": 0.0, "trade_df": trade_df}
    allocation = float(capital) / max(int(max_positions), 1)
    trade_df["P&L ₹"] = trade_df["Return %"] / 100 * allocation
    wins = trade_df[trade_df["Return %"] > 0]
    pnl = float(trade_df["P&L ₹"].sum())
    years = pd.to_datetime(trade_df["Entry Date"]).dt.year.nunique()
    return {
        "data_years": f"Up to 5 years (actual available history per stock)",
        "trades": int(len(trade_df)),
        "win_rate": float(len(wins) / len(trade_df) * 100),
        "pnl": pnl,
        "trade_df": trade_df.sort_values("Entry Date", ascending=False).reset_index(drop=True),
    }
...
@st.cache_data(ttl=3600, show_spinner=False)
def quality_swing_universe():
    """Broad NSE quality universe for swing trading.

    This intentionally replaces the old Combined universe. It starts from
    the complete NSE equity master and keeps liquid, established stocks.
    The technical six-condition strategy remains the final entry filter.
    """
    all_symbols = sorted(set(load_nse_equity_symbols()))
    if not all_symbols:
        return sorted(set(load_nifty500()) | set(load_fno_symbols()))

    # Fast current-market-quality filter. We avoid a hard ₹10,000 Cr cutoff
    # so good mid-cap swing candidates are not unnecessarily excluded.
    kept = []
    for symbol in all_symbols:
        try:
            ...
            price = safe_float(info.get("lastPrice"), 0)
            if market_cap >= 1_000_000_000 and price >= 20:
                kept.append(symbol)
        except Exception:
            continue

    # If Yahoo metadata is temporarily unavailable, do not return an empty
    # universe; fall back to the established NSE sets.
    if len(kept) < 150:
        kept = sorted(set(load_nifty500()) | set(load_fno_symbols()))
...
def selected_symbols(universe, limit_count):
    ...
    elif universe == "Quality Swing Universe":
        symbols = quality_swing_universe()
    else:
        symbols = []
...
if "v26_results" not in st.session_state:
    st.session_state["v26_results"] = pd.DataFrame()
if "v26_previous" not in st.session_state:
    st.session_state["v26_previous"] = pd.DataFrame()
if "v26_regime" not in st.session_state:
    st.session_state["v26_regime"] = {"regime": "NOT USED", "score": 0, "close": np.nan, "ma20": np.nan, "ma50": np.nan}
if "v26_backtest" not in st.session_state:
    st.session_state["v26_backtest"] = {}
if "v26_mode" not in st.session_state:
    st.session_state["v26_mode"] = scanner_mode

if scanner_mode != st.session_state["v26_mode"]:
    st.session_state["v26_results"] = pd.DataFrame()
    st.session_state["v26_previous"] = pd.DataFrame()
    st.session_state["v26_mode"] = scanner_mode
...
if run_scan:
    if scanner_mode == "📈 Swing":
        ...
        st.session_state["v26_regime"] = regime
        st.session_state["v26_previous"] = pd.DataFrame()
    elif scanner_mode == "⚡ Intraday":
    ...
    st.session_state["v26_results"] = result
    st.session_state["v26_last_scan"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

results = st.session_state["v26_results"]
previous_df = st.session_state["v26_previous"]
regime = st.session_state["v26_regime"]

# ============================================================
# HEADER
# ============================================================

st.title("📈 TradeQuest V26")
st.caption("Single-Scanner Live Entry + Portfolio Scanner")
st.info(
    f"Running: **{scanner_mode}**  |  Capital: **₹{trading_capital:,.0f}**  |  "
    f"Last scan: **{st.session_state.get('v26_last_scan', 'Not run')}**"
)
...
if scanner_mode == "📈 Swing":
    ...
    if results.empty:
        ...
        near = results[results["Status"] == "🟡 NEAR ENTRY — Watch Closely"].copy().sort_values(["Score", "Conditions Met"], ascending=False).head(20)
        watch = results[results["Status"] == "🔵 WATCH — Setup Developing"].copy().sort_values(["Score", "Conditions Met"], ascending=False).head(10)
        ...
        # READY does NOT mean automatically saved.
        # The user explicitly selects what was actually bought.
        # --------------------------------------------------------
        st.subheader("💾 SAVE SELECTED READY STOCKS AS ACTIVE TRADES")
        if ready.empty:
            st.info("No READY stock to save. READY signals are not saved automatically.")
        else:
            selectable = ready.sort_values(["Score", "R:R"], ascending=False).drop_duplicates("Symbol").copy()
            selectable.insert(0, "Save as Active Trade", False)
            selectable["Actual Buy Price"] = selectable["Entry"]
            selectable["Qty"] = 0
            for i in selectable.index:
                entry_i = safe_float(selectable.at[i, "Entry"], 0)
                sl_i = safe_float(selectable.at[i, "SL"], 0)
                risk_budget = trading_capital * (RISK_PERCENT / 100.0)
                suggested_qty = int(risk_budget / max(entry_i - sl_i, 0.01)) if entry_i > 0 else 0
                suggested_qty = min(suggested_qty, int(trading_capital / entry_i)) if entry_i > 0 else 0
                selectable.at[i, "Qty"] = max(1, suggested_qty) if entry_i > 0 else 0

            edit_cols = [x for x in ["Save as Active Trade", "Symbol", "Current", "Actual Buy Price", "Qty", "SL", "T1", "Conditions Met", "RSI", "Vol x", "Score"] if x in selectable.columns]
            edited = st.data_editor(
                selectable[edit_cols],
                use_container_width=True,
                hide_index=True,
                key="v26_ready_editor",
                column_config={
                    "Save as Active Trade": st.column_config.CheckboxColumn("Save as Active Trade", default=False),
                    "Actual Buy Price": st.column_config.NumberColumn("Actual Buy Price", min_value=0.01, step=0.05),
                    "Qty": st.column_config.NumberColumn("Qty", min_value=1, step=1),
                },
                disabled=[x for x in edit_cols if x not in ["Save as Active Trade", "Actual Buy Price", "Qty"]],
            )
            if st.button("💾 SAVE SELECTED AS ACTIVE TRADES", type="primary", key="v26_save_active"):
                chosen = edited[edited["Save as Active Trade"] == True].copy()
                if chosen.empty:
                    st.warning("Select at least one stock that you actually bought.")
                else:
                    active = load_active_trades()
                    added = 0
                    for _, row in chosen.iterrows():
                        symbol = clean_symbol(row.get("Symbol", ""))
                        if any(clean_symbol(x.get("symbol", "")) == symbol and x.get("status") == "ACTIVE" for x in active):
                            continue
                        buy_price = safe_float(row.get("Actual Buy Price"), safe_float(row.get("Entry"), 0))
                        qty = int(max(1, safe_float(row.get("Qty"), 1)))
                        active.append({
                            "symbol": symbol,
                            "scanner": "📈 Swing",
                            "status": "ACTIVE",
                            ...
                            "buy_price": buy_price,
                            "qty": qty,
                            "sl": buy_price * (1 - STOP_PERCENT / 100),
                            "target": buy_price * (1 + TARGET_PERCENT / 100),
                            "max_hold_days": MAX_HOLDING_DAYS,
                        })
                        added += 1
                    save_active_trades(active)
                    st.success(f"Saved {added} selected trade(s). They will remain tracked until Target, Stop Loss, or 20 trading days.")

        active_rows = update_active_trades(results)
        st.subheader("📌 ACTIVE TRADES — Only Stocks You Selected/Bought")
        if not active_rows:
            st.info("No active trades saved. READY stocks are not automatically carried forward.")
        else:
            active_df = pd.DataFrame(active_rows)
            display_cols = [x for x in ["Symbol", "Buy Date", "Buy Price", "Current", "Qty", "P/L ₹", "P/L %", "SL", "Target", "Days Held", "Status"] if x in active_df.columns]
            st.dataframe(active_df[display_cols], use_container_width=True, hide_index=True)

        # --------------------------------------------------------
        # 5-YEAR HISTORICAL BACKTEST
        # --------------------------------------------------------
        st.subheader("📊 5-YEAR HISTORICAL BACKTEST — Same Six-Condition Strategy")
        backtest_count = st.slider("Backtest stocks", 20, min(500, max(20, len(selected_symbols(selected_universe, max_stocks)))), min(100, max(20, len(selected_symbols(selected_universe, max_stocks)))), 20, key="v26_bt_count")
        if st.button("▶️ RUN 5-YEAR BACKTEST", key="v26_backtest"):
            bt_symbols = selected_symbols(selected_universe, backtest_count)
            bt = run_five_year_backtest(bt_symbols, trading_capital, int(max_positions))
            st.session_state["v26_backtest"] = bt
        bt = st.session_state.get("v26_backtest", {})
        if bt:
            m1,m2,m3,m4 = st.columns(4)
            m1.metric("Data", bt.get("data_years", ""))
            m2.metric("Trades", bt.get("trades", 0))
            m3.metric("Win Rate", f"{bt.get('win_rate', 0):.2f}%")
            m4.metric("P&L", f"₹{bt.get('pnl', 0):,.0f}")
            if bt.get("trade_df") is not None and not bt["trade_df"].empty:
                st.dataframe(bt["trade_df"].head(100), use_container_width=True, hide_index=True)
            st.caption("Backtest uses actual available history. Stocks with less than 5 years are labeled with their available period; no synthetic history is created.")
...
- **Swing:** V19/V20 six-condition entry strategy: Price > MA20, MA20 > MA50, RSI 50-70, MACD > Signal, Volume >= 1.5x, 50D Breakout.
...
- **Active Trades:** only stocks you explicitly select after buying are persisted until Target, Stop Loss, or 20 trading days.
- **Quality Swing Universe:** broad NSE quality universe; no fixed ₹10,000 Cr cutoff.
""")
...
if not results.empty:
    st.download_button(
        ...
        file_name={
            "📈 Swing":"tradequest_v26_swing.csv",
            "⚡ Intraday":"tradequest_v26_intraday.csv",
            "🎯 Stock F&O":"tradequest_v26_stock_fno.csv",
            "📊 Index F&O":"tradequest_v26_index_fno.csv",
        }[scanner_mode],
        mime="text/csv",
    )

st.caption("TradeQuest V26 is a quantitative screening/research tool. It is not personalized investment advice or a guarantee of execution/profit.")
